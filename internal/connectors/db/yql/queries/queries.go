...
$last_backup_id = SELECT schedule_id AS schedule_id_2, MAX_BY(b.id, b.completed_at) AS last_backup_id, MAX_BY(b.status, b.completed_at) AS last_backup_status FROM Backups AS b GROUP BY schedule_id;
...
$last_backup = SELECT
    <|
        id: MAX_BY(b.id, b.completed_at),
        status: MAX_BY(b.status, b.completed_at)
    |> FROM Backups AS b WHERE b.schedule_id = $schedule_id;

SELECT s.*, $last_backup.id AS last_backup_id, $last_backup.status AS last_backup_status, $rpo_info.recovery_point AS recovery_point, $rpo_info.last_successful_backup_id AS last_successful_backup_id FROM BackupSchedules AS s WHERE s.id = $schedule_id
`, types.BackupStateAvailable,
	)
